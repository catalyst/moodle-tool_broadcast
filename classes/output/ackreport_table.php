 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class ackreport_table extends table_sql implements renderable {
    ...
    public function __construct(string $uniqueid, int $broadcastid, int $contextid, string $baseurl, int $page = 0) {
        ...
        $this->set_attribute('id', 'tool_broadcast_ackreport_table');
        ...
        $headers[] = get_string('report:location', 'tool_broadcast');
        $columns[] = 'contextid';

        $headers[] = get_string('report:acktime', 'tool_broadcast');
        $columns[] = 'acktime';
